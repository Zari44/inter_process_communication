...
int main(int argc, char *argv[])
{
  ...
  int parent_to_child = std::atoi(argv[1]);
  int child_to_parent = std::atoi(argv[2]);
  char buffer[BUFSIZ + 1];
  memset(buffer, '\0', sizeof(buffer));
  std::cout << "parent_to_child = " << parent_to_child << std::endl;
  std::cout << "child_to_parent = " << child_to_parent << std::endl;

  while(strcmp(buffer, "quit")) {
    int data_processed = 0;
    // while (read(parent_to_child, buffer, BUFSIZ)){
      // data_processed++;
    // }
    data_processed = read(parent_to_child, buffer, BUFSIZ);
